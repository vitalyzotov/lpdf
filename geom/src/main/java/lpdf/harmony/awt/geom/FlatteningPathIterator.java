...
public class FlatteningPathIterator implements PathIterator {
    ...
    public FlatteningPathIterator(PathIterator path, double flatness, int limit) {
        if (flatness < 0.0) {
            ...
            throw new IllegalArgumentException("Flatness is less then zero"); //$NON-NLS-1$
        }
        if (limit < 0) {
            // awt.207=Limit is less then zero
            throw new IllegalArgumentException("Limit is less then zero"); //$NON-NLS-1$
        }
        if (path == null) {
            // awt.208=Path is null
            throw new NullPointerException("Path is null"); //$NON-NLS-1$
        }
        this.p = path;
    ...
    public int currentSegment(float[] coords) {
        if (isDone()) {
            ...
            throw new NoSuchElementException("Iterator out of bounds"); //$NON-NLS-1$
        }
        evaluate();
        int type = bufType;
        if (type != SEG_CLOSE) {
